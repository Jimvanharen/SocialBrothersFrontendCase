...
export default function Home({categoriesProp}) {
  ...
  return (
    <div className="max-w-full h-full">
      ...
      <div className="main-container flex gap-32 mt-12 sm:flex-row flex-col">
        <div className="sm:ml-8 mx-auto h-[600px] addpost-box w-2/4 bg-gray-200 flex justify-start min-w-fit sm:p-0 p-8">
          <form onSubmit={postBericht} className="mt-4 mx-auto flex flex-col gap-7">
            ...
            <input id="categoriesInput" list="categories" required onChange={(e) => setCategorie(e.target.value)}/>
            <datalist id="categories">
              {categoriesProp.map(categorie => (
                <option value={categorie.name} key={categorie.id}/>
              ))}
            </datalist>
            <label htmlFor="berichtPost">Bericht</label>
            <textarea id="berichtPost" className="border-2" required onChange={(e) => setBericht(e.target.value)}></textarea>
        ...
        <div className="blog-collection-box w-2/4 bg-gray-200 sm:mr-8 sm:my-0 sm:ml-0 m-auto h-2/3 pb-4">
          <div className="grid grid-cols-2">
            <Blog title="test" category="flagged" text="blabla" image="/SBlogo.svg" />
            <Blog title="test" category="flagged" text="blabla" image="/SBlogo.svg" />
            <Blog title="test" category="flagged" text="blabla" image="/SBlogo.svg" />
            <Blog title="test" category="flagged" text="blabla" image="/SBlogo.svg" />
